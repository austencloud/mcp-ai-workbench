...
async function testReasoningSystem() {
  ...
  try {
    ...
    const basicTest = await axios.post(`${BASE_URL}/rpc`, {
      ...
      params: {
        problem:
          "What are the main causes of climate change and how do they interconnect?",
    ...
    if (basicTest.data.result.success) {
      ...
      console.log(
        `📊 Confidence: ${basicTest.data.result.trace.confidence.toFixed(2)}`
      );
      console.log(
        `🔧 Tools used: ${basicTest.data.result.trace.toolsUsed.join(", ")}`
      );
      console.log(
        `📝 Answer: ${basicTest.data.result.trace.finalAnswer.substring(
          0,
          100
        )}...\n`
      );
    ...
    if (mathTest.data.result.success) {
      ...
      console.log(
        `📊 Confidence: ${mathTest.data.result.trace.confidence.toFixed(2)}`
      );
      console.log(
        `🔧 Tools used: ${mathTest.data.result.trace.toolsUsed.join(", ")}`
      );
      console.log(
        `📝 Answer: ${mathTest.data.result.trace.finalAnswer.substring(
          0,
          100
        )}...\n`
      );
    ...
    const webTest = await axios.post(`${BASE_URL}/rpc`, {
      ...
      params: {
        problem:
          "What are the latest developments in AI reasoning models in 2024?",
        type: "web_research",
        requireExplanation: true,
      },
      id: 3,
    ...
    if (webTest.data.result.success) {
      ...
      console.log(
        `📊 Confidence: ${webTest.data.result.trace.confidence.toFixed(2)}`
      );
      console.log(
        `🔧 Tools used: ${webTest.data.result.trace.toolsUsed.join(", ")}`
      );
      console.log(
        `📝 Answer: ${webTest.data.result.trace.finalAnswer.substring(
          0,
          100
        )}...\n`
      );
    ...
    if (typesTest.data.result.success) {
      ...
      console.log(
        `📋 Available types: ${typesTest.data.result.types.join(", ")}\n`
      );
    } else {
    ...
    if (trainingTest.data.result.success) {
      ...
      console.log(
        `📊 Generated ${trainingTest.data.result.examples.length} examples`
      );
    ...
    if (chatTest.data.result.success) {
      ...
      console.log(
        `📝 Response: ${chatTest.data.result.message.content.substring(
          0,
          100
        )}...`
      );
      if (chatTest.data.result.reasoning) {
        console.log(
          `🧠 Reasoning steps: ${chatTest.data.result.reasoning.steps.length}`
        );
        console.log(
          `📊 Confidence: ${chatTest.data.result.reasoning.confidence.toFixed(
            2
          )}\n`
        );
      }
    ...
    if (statsTest.data.result.success) {
      ...
      console.log(
        `📊 Success rate: ${(statsTest.data.result.successRate * 100).toFixed(
          1
        )}%`
      );
      console.log(
        `🔧 Popular tools: ${statsTest.data.result.popularTools.join(", ")}\n`
      );
    } else {
