...
export class MemoryController {
  ...
  async searchMemories(params: {
    ...
    try {
      ...
      // Build context for search - ensure IDs are strings
      const context: Partial<MemoryContext> = {};
      if (userId) context.userId = userId;
      if (workspaceId) context.workspaceId = String(workspaceId);
      if (conversationId) context.conversationId = String(conversationId);
