<script lang="ts">
  import { memoryService, getUserInsights, getRecentMemories, getIsMemoryLoading } from '$lib/services/memoryService.svelte';
  ...
  interface Props {
    ...
    onmemoryContext?: (event: { detail: { context: string; memories: MemoryItem[] } }) => void;
    onmemoryStored?: (event: { detail: { memoryId: string; type: string } }) => void;
    onepisodeRecorded?: (event: { detail: { episodeId: string } }) => void;
  }
  ...
  let relevantMemories = $state<MemoryItem[]>([]);
  let memoryContext = $state('');
  let showMemoryPanel = $state(false);
  let adaptationSuggestions = $state('');
  ...
  async function analyzeCurrentMessage() {
    ...
    try {
      ...
      if (relevantMemories.length > 0) {
        ...
        onmemoryContext?.({ detail: { context: memoryContext, memories: relevantMemories } });
      }
    } catch (error) {
  ...
  async function rememberCurrentMessage(type: string = 'observation', importance: number = 0.5) {
    ...
    try {
      ...
      if (response.success) {
        onmemoryStored?.({ detail: { memoryId: response.memoryId, type } });
  ...
  async function recordExperience(event: string, outcome: string, success: boolean = true) {
    try {
      ...
      if (response.success) {
        onepisodeRecorded?.({ detail: { episodeId: response.memoryId } });
  ...
  // Set up memory service context on mount
  $effect(() => {
...
<div class="memory-integration">
  ...
  {#if isAnalyzing || getIsMemoryLoading()}
  ...
  {#if getUserInsights().length > 0}
    <div class="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mb-3">
      ...
      <div class="space-y-1">
        {#each getUserInsights().slice(0, 2) as insight}
  ...
  <div class="flex flex-wrap gap-2 mb-3">
    <button
      onclick={() => rememberCurrentMessage('fact', 0.7)}
    ...
    <button
      onclick={() => rememberCurrentMessage('observation', 0.5)}
      disabled={!currentMessage.trim()}
    ...
    <button
      onclick={() => recordExperience(currentMessage, 'User interaction', true)}
      disabled={!currentMessage.trim()}
  ...
  {#if getRecentMemories().length > 0}
    <div class="text-xs text-gray-400 mb-2">
      Recent activity: {getRecentMemories().length} memories in the last 7 days
    </div>
  {/if}
</div>
