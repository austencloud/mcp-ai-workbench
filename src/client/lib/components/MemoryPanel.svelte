<script lang="ts">
  ...
  let { isOpen = $bindable(false), userId = 'default-user' }: Props = $props();

  let searchQuery = $state('');
  let memories = $state<MemorySearchResult[]>([]);
  let loading = $state(false);
  let selectedMemory = $state<MemoryItem | null>(null);
  let activeTab = $state('search'); // 'search', 'timeline', 'insights', 'preferences'
  let userInsights = $state<string[]>([]);
  let episodicTimeline = $state<any[]>([]);
  ...
  async function loadRecentMemories() {
    ...
    try {
      ...
      if (response.success) {
        memories = response.memories || [];
        console.log(`📚 Loaded ${memories.length} recent memories`);
      } else {
        console.error('Failed to load recent memories:', response);
      }
    } catch (error) {
  ...
  $effect(() => {
    if (isOpen) {
      loadUserInsights();
      // Auto-load recent memories on the search tab
      if (activeTab === 'search') {
        loadRecentMemories();
      }
    }
  ...
  $effect(() => {
    if (isOpen && activeTab === 'insights') {
      loadUserInsights();
    } else if (isOpen && activeTab === 'search' && memories.length === 0) {
      // Load recent memories if search tab is empty
      loadRecentMemories();
            ...
            <div class="flex-1 overflow-y-auto p-6">
                ...
                <div class="space-y-4">
                  {#each memories as result}
                    <button
                      ...
                      onclick={() => selectedMemory = result.memory}
              ...
              {:else}
                <div class="text-center text-gray-400 py-12">
                  ...
                  <p class="mb-2">Your recent memories will appear here</p>
                  <p class="text-sm text-gray-500">Search above or interact with the AI to create memories</p>
                </div>
              {/if}
            </div>
          </div>
