<script lang="ts">
  ...
  import { memoryService, memoryStats, userInsights, episodicTimeline, userPreferences } from '$lib/services/memoryService.svelte';
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          {#each Object.entries(userPreferences().reduce((acc, pref) => {
            ...
            <div>
              ...
              <div class="space-y-1">
                {#each (prefs as any[]).slice(0, 3) as pref}
    ...
    {#if memoryStats()?.systemHealth?.performanceMetrics}
      <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div class="text-center">
            <p class="text-2xl font-bold text-green-400">
              {((memoryStats()?.systemHealth?.performanceMetrics?.storageUsage || 0) / 1024 / 1024).toFixed(1)}MB
            </p>
            <p class="text-gray-400 text-sm">Storage Used</p>
          </div>
          <div class="text-center">
            <p class="text-2xl font-bold text-purple-400">
              {((memoryStats()?.systemHealth?.performanceMetrics?.indexHealth || 0) * 100).toFixed(0)}%
            </p>
